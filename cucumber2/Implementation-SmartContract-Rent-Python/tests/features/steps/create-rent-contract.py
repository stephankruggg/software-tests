...
address = "0x8ebfa3509E6EBBf83b27652DfE618c9A920Bc3fc"
private_key = "0x57e3227f384c726dabc0d777c953482b5fa3ec581f3529ec748fa93da8632fbf"
...
def __deploy_contract(landlord, tenant, tenantAddress, contractTotalValue, monthlyValue,
    ...
    with open("/home/stephan/git/software-tests/exercise11/Implementation-SmartContract-Rent-Python/src/resources/RentContract.sol", "r") as file:
...
@step("water payment is done")
def step_impl(context):
    transaction = smart_contract.functions.make_water_payment().build_transaction({"chainId": chain_id,
                                                                         "gasPrice": w3.eth.gas_price,
                                                                         "from": address,
                                                                         "nonce": w3.eth.get_transaction_count(address)})
    sign_transaction = w3.eth.account.sign_transaction(transaction, private_key=private_key)
...
@step("taxes payment is done")
def step_impl(context):
    transaction = smart_contract.functions.make_taxes_payment().build_transaction({"chainId": chain_id,
                                                                         "gasPrice": w3.eth.gas_price,
                                                                         "from": address,
                                                                         "nonce": w3.eth.get_transaction_count(address)})
    sign_transaction = w3.eth.account.sign_transaction(transaction, private_key=private_key)
...
@step("the water paid is {water_payment}")
def step_impl(context, water_payment):
    TestCase.assertEqual(TestCase(), False, smart_contract.functions.getWaterPaid().call())

@step("the taxes paid is {taxes_payment}")
def step_impl(context, taxes_payment):
    TestCase.assertEqual(TestCase(), False, smart_contract.functions.getTaxesPaid().call())

@step("the penalty is {penalty}")
def step_impl(context, penalty):
    TestCase.assertEqual(TestCase(), int(penalty), smart_contract.functions.getPenalty().call())
